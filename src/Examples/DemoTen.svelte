...
<script>
  ...
  export const more = `This example is similar to [Demo Five](/demos/demoFive) in that it doesn't use a config (though you could do that too), and sends in each base in a 'bases' array, each with its own base and API keys. The second base is the "Editor Public" base we'll use in the Write-capable examples. Keep in mind that you might run into API limits pretty soon if you do this in production, so this is either best for prototype development, or for a server to cache it somewhere`
  /*
